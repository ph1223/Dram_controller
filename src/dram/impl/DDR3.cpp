...
public:
  inline static const std::map<std::string, Organization> org_presets = {
      ...
      {"DDR3_4Gb_x128", {2 << 12, 128, {1, 1, 4, 1 << 14, 1 << 10}}},
      //
  ...
  inline static const std::map<std::string, std::vector<int>> timing_presets = {
      //   name       rate    nBL  nCL  nRCD  nRP   nRAS  nRC   nWR  nRTP nCWL
      //   nCCD  nRRD  nWTR  nFAW  nRFC nREFI  nCS  tCK_ps
      {"DDR3_800D",
       {800, 4, 5, 5, 5, 15, 20, 6, 4, 9, 4, -1, 4, -1, -1, -1, 2, 2500}},
      {"DDR3_800E",
       {800, 4, 5, 5, 5, 15, 20, 6, 4, 9, 4, -1, 4, -1, -1, -1, 2, 2500}},
      {"DDR3_1066E",
       {1066, 4, 6, 6, 6, 20, 26, 8, 4, 9, 4, -1, 4, -1, -1, -1, 2, 1875}},
      {"DDR3_1066F",
       {1066, 4, 7, 7, 7, 20, 27, 8, 4, 9, 4, -1, 4, -1, -1, -1, 2, 1875}},
      {"DDR3_1066G",
       {1066, 4, 8, 8, 8, 20, 28, 8, 4, 9, 4, -1, 4, -1, -1, -1, 2, 1875}},
      {"DDR3_1333G",
       {1333, 4, 8, 8, 8, 24, 32, 10, 5, 9, 4, -1, 5, -1, -1, -1, 2, 1500}},
      {"DDR3_1333H",
       {1333, 4, 9, 9, 9, 24, 33, 10, 5, 9, 4, -1, 5, -1, -1, -1, 2, 1500}},
      //   name
      {"DDR3_1600H",
       {1600, 4, 9, 9, 9, 28, 37, 12, 6, 9, 4, -1, 6, -1, -1, -1, 2, 1250}},
      {"DDR3_1600J",
      ...
      {"DDR3_1866K",
       {1866, 4, 11, 11, 11, 32, 43, 14, 7, 9, 4, -1, 7, -1, -1, -1, 2, 1071}},
      {"DDR3_1866L",
       {1866, 4, 12, 12, 12, 32, 44, 14, 7, 9, 4, -1, 7, -1, -1, -1, 2, 1071}},
      {"DDR3_2133L",
       {2133, 4, 12, 12, 12, 36, 48, 16, 8, 9, 4, -1, 8, -1, -1, -1, 2, 937}},
      {"DDR3_2133M",
       {2133, 4, 13, 13, 13, 36, 49, 16, 8, 9, 4, -1, 8, -1, -1, -1, 2, 937}},
  };
  ...
  inline static constexpr ImplDef m_commands = {
      "ACT", "PRE", "PREA", "RD", "WR", "RDA", "WRA", "REFab",
  };
  ...
  inline static constexpr ImplDef m_timings = {
      ...
      "nCWL", "nCCD", "nRRD", "nWTR", "nFAW", "nRFC", "nREFI", "nCS", "tCK_ps"};
...
public:
  struct Node : public DRAMNodeBase<DDR3> {
    Node(DDR3 *dram, Node *parent, int level, int id)
        : DRAMNodeBase<DDR3>(dram, parent, level, id){};
  };
...
private:
  void set_organization() {
    ...
    // Sanity check: is the calculated chip density the same as the provided
    // one?
    ...
    if (m_organization.density != _density) {
      throw ConfigurationError("Calculated {} chip density {} Mb does not "
    ...
    populate_timingcons(
        this, {
                  ...
                  {.level = "rank",
                   .preceding = {"WR", "WRA"},
                   .following = {"RD", "RDA"},
                   .latency = V("nCWL") + V("nBL") + V("nWTR")},
                  /// CAS <-> CAS between sibling ranks, nCS (rank switching) is
                  /// needed for new DQS
                  {.level = "rank",
                   .preceding = {"RD", "RDA"},
                  ...
                  {.level = "rank",
                   .preceding = {"WR", "WRA"},
                   .following = {"RD", "RDA"},
                   .latency = V("nCL") + V("nBL") + V("nCS") - V("nCWL"),
                   .is_sibling = true},
                  /// CAS <-> PREab
                  {.level = "rank",
                   .preceding = {"RD"},
...
#undef V
  ...
  void set_actions() {
    ...
    m_actions[m_levels["rank"]][m_commands["PREA"]] =
        Lambdas::Action::Rank::PREab<DDR3>;

    // Bank actions
    m_actions[m_levels["bank"]][m_commands["ACT"]] =
        Lambdas::Action::Bank::ACT<DDR3>;
    m_actions[m_levels["bank"]][m_commands["PRE"]] =
        Lambdas::Action::Bank::PRE<DDR3>;
  ...
  void set_preqs() {
    ...
    m_preqs[m_levels["rank"]][m_commands["REFab"]] =
        Lambdas::Preq::Rank::RequireAllBanksClosed<DDR3>;

    // Bank actions
    m_preqs[m_levels["bank"]][m_commands["RD"]] =
        Lambdas::Preq::Bank::RequireRowOpen<DDR3>;
  ...
  void set_rowopens() {
    ...
    m_rowopens[m_levels["bank"]][m_commands["RD"]] =
        Lambdas::RowOpen::Bank::RDWR<DDR3>;
