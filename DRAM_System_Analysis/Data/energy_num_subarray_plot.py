...
# Split and plot total_energy (converted from nJ to mJ), grouped and color-coded by subarray count
def split_and_plot(df):
    ...
    # Convert total_energy from nJ to mJ (divide by 1e6)
    df['total_energy_mJ'] = df['total_energy'] / 1e6

    # Sort within each group by total_energy_mJ ascending
    group_ar = df[df['Group'] == 'Auto Refresh'].sort_values(by='total_energy_mJ').reset_index(drop=True)
    group_non_ar = df[df['Group'] == 'WUPR'].sort_values(by='total_energy_mJ').reset_index(drop=True)
