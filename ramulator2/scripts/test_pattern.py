...
def generate_st_ld_trace(filename,filename2,write_weight_lines,num_operation_decode_prefill,num_lines,gen_stall,load_store_switch_threshold,gen_load_store_pattern = False):
    address = 0
    gen_row_bits = 0
    ...
    with open(filename, 'w') as file,open(filename2,'w') as file2:
        for line in range(write_weight_lines):
        ...
        for i in range(num_operation_decode_prefill):
            for line in range(write_address):
                operation = 'LD'
                # Load for a number of cycles, then read for a number of cycles
                gen_channel_num = 0
                gen_row_bits    = line // column_partitions
...
for i in range(num_traces):
    ...
    generate_st_ld_trace(filename,filename2,write_weight_lines,num_operation_decode_prefill,num_lines,gen_stall,load_store_switch_threshold = load_store_switch_threshold,gen_load_store_pattern = gen_load_store_pattern)
